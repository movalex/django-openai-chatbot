...
class TestChatbotView:
    ...
    def test_chatbot_invalid_room_returns_404(self, authenticated_client):
        ...
        fake_id = uuid.uuid4()
        response = authenticated_client.get(
            reverse("chat_room", args=[fake_id])
        )
        assert response.status_code == 404

    def test_chatbot_cannot_access_other_user_room(
        ...
        response = authenticated_client.get(
            reverse("chat_room", args=[other_room.id])
        )
        assert response.status_code == 404

    def test_chatbot_updates_last_opened_chat(
        self, authenticated_client, user, chat_room
    ):
    ...
    def test_chatbot_displays_existing_chats(
        self, authenticated_client, user, chat_room
    ):
    ...
    def test_chatbot_post_without_model_id(
        self, authenticated_client, chat_room
    ):
...
class TestGetChatRooms:
    ...
    def test_get_chat_rooms_returns_user_rooms(
    ...
    def test_get_chat_rooms_excludes_hidden(
        self, authenticated_client, user
    ):
    ...
    def test_get_chat_rooms_ordered_by_created_at(
        self, authenticated_client, user
    ):
...
class TestArchiveChat:
    ...
    def test_archive_chat_requires_authentication(self, client, chat_room):
        ...
        response = client.post(
            reverse("archive_chat", args=[chat_room.id])
        )
        assert response.status_code == 401
    ...
    def test_archive_chat_not_found(self, authenticated_client):
        ...
        import uuid
        response = authenticated_client.post(
            reverse("archive_chat", args=[uuid.uuid4()])
        )
        assert response.status_code == 404

    def test_archive_chat_cannot_archive_other_user_room(
        self, authenticated_client, another_user
    ):
        ...
        response = authenticated_client.post(
            reverse("archive_chat", args=[other_room.id])
        )
        assert response.status_code == 404
...
class TestChatContext:
    ...
    def test_get_chat_context_handles_invalid_json(
        self, user, chat_room
    ):
